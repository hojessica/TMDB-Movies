...
myplots[[8]]
# Here, the plot for August is fascinating. One can clearly notice that revenue decreases as the days go by. A possible reason for this is the return to school for families and their children.
...
# Thriller Movies
fullmovies_thrill <- fullmovies[fullmovies$Thriller==1 & fullmovies$runtime<170, ]
set.seed(216)
testmse <- rep(0, 10)
df_mse <- data.frame(matrix(ncol = 2, nrow = 10))
colnames(df_mse) <- c("Degree", "Root MSE")
...
```{r, echo = F}
#Results Table for Q1

#Final Table
result=rbind(bud.rev, pop.rev)
result[1,1] = "Budget Degree 3"
result[2,1] = "Popularity Degree 5"
colnames(result)[colnames(result)=="Root.MSE"] <- "Root MSE"
result = result[order(result$`Root MSE`), ]

#Interactions Table
#df_mse2

#Convert these dataframes into HTML tables and aligh them side-by-side
result %>%
  kable() %>%
    kable_styling(bootstrap_options = c("striped", "hover"), full_width = F)
...
One area where we observed a few striking outliers was in runtime. While most movies are 90 to 120 minutes long, there were a handful of films that were longer than 160 minutes. After understanding how revenue works a bit better from the previous question, our team decided to explore the other factor production companies are concerned about – popularity. Specifically, we were curious to learn the runtimes that were ideal to make a popular movie. We expected the peak for Comedy in popularity to be the shortest runtime and for Fantasy to be the longest. After visualizing the data we found that Comedy movies that are near 100 minutes long are ideal and popularity seems to flatline as movies in this genre get longer. In contrast, the most popular movies for Action and Fantasy are over 150 minutes long.

These findings start an important investigation into the performance of a film based on genre. While ideal release dates, budget and runtime can be maximized for the ideal film, there are some genres where these variables behave vastly different to predict popularity and revenue. Some of the most popular and successful movies have been outliers in the explanatory variables. By having an in-depth understanding of these observations, the industry can be mindful of when breaking traditional methods. Additionally, the amount of money spent is directly correlated with the length of a movie which determines the time it takes to film. By having a better understanding of the ideal length for a movie based on genre, production companies and directors can use their funds wisely to make a more popular film. Moving forward, we think it’d be interesting to investigate how genres have changed over time. As the entertainment industry keeps on changing, we’d want to know which genres are the ones being disrupted the most and which will keep performing as they have in the past. Our data set lacked significant observations from earlier years but by having more observations from before the 21st century, we can anticipate consumer behaviors for the future.
